"""Bass transcription via Basic Pitch for a synchronized bass roll view."""
...
def _empty_events() -> dict:
    return {"_model": "Basic Pitch", "notes": []}
...
def midi_to_events(midi_path: str | Path) -> dict:
    ...
    for instrument in midi.instruments:
        ...
        for note in instrument.notes:
            ...
            if pitch < BASS_LOW or pitch > BASS_HIGH:
                continue
            notes.append({
                "pitch": pitch,
    ...
    return {"_model": "Basic Pitch dominant bass", "notes": mono, "_raw_notes": len(notes)}


def dominant_monophonic(notes: list[dict], step: float = 0.05, min_length: float = 0.08) -> list[dict]:
    if not notes:
        return []
    clean = []
    for note in notes:
        try:
            ...
            continue
        if end <= start or pitch < BASS_LOW or pitch > BASS_HIGH:
            continue
    ...
    while t < last:
        ...
        if active:
            # Basic Pitch can mistake upper harmonics for notes. Velocity is the main cue,
            # with a small preference for lower bass notes when confidence is close.
            best = max(active, key=lambda n: (n["velocity"] - max(0, n["pitch"] - 52) * 1.5, n["velocity"], -n["pitch"]))
...
def _find_midi(output_dir: Path, source: Path) -> Path:
    candidates = [
        output_dir / f"{source.stem}_basic_pitch.mid",
        output_dir / f"{source.stem}.mid",
    ]
...
def estimate_bass_events(path: str | Path, folder: str | Path) -> dict:
    path = Path(path)
    folder = Path(folder)
    out_dir = folder / ".basic-pitch-bass"
    if out_dir.exists():
...
def load_or_estimate(folder: str | Path, force: bool = False) -> dict:
    folder = Path(folder)
    cache = folder / "bass-midi.json"
    if cache.is_file() and not force:
        data = json.loads(cache.read_text(encoding="utf-8"))
        data.setdefault("_model", "Basic Pitch")
        data.setdefault("notes", [])
        if "dominant" not in data.get("_model", "").lower():
            raw_count = len(data.get("notes", []))
            data["notes"] = dominant_monophonic(data.get("notes", []))
            data["_model"] = "Basic Pitch dominant bass"
            data["_raw_notes"] = raw_count
        return data
    bass = folder / "bass.wav"
    if not bass.is_file():
        return _empty_events()
    events = estimate_bass_events(bass, folder)
